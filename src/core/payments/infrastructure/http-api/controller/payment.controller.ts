...
export class PaymentController {
  ...
  @Get(':id')
  async findPaymentByIdRequest(
    @Param() findPaymentByIdDto: FindPaymentByIdDto,
  ): Promise<{ payment: PrimitivePayment }> {
    return await this.findPaymentByIdUseCase.execute(findPaymentByIdDto);
  }

  @Get('customer/:customerId')
  async findPaymentByCustomerIdRequest(
    @Param() findPaymentByCustomerIdDto: FindPaymentByCustomerIdDto,
  ): Promise<{ payments: PrimitivePayment[] }> {
    return await this.findPaymentByCustomerIdUseCase.execute(
      findPaymentByCustomerIdDto,
    );
  }
}
