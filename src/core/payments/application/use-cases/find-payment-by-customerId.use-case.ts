...
export class FindPaymentByCustomerIdUseCase {
	...
	async execute(findPaymentByCustomerIdDto: FindPaymentByCustomerIdDto): Promise<{payments: PrimitivePayment[]}> {
		const payments = await this.paymentRepository.findByCustomerId(findPaymentByCustomerIdDto.customerId);
